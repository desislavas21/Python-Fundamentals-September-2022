...
overall = 0
for i in range(n):
    price_per_capsule, days, needed_capsule = float(input()), int(input()), int(input())
    rules = [0.01 <= price_per_capsule <= 100, 1 <= days <= 31, 1 <= needed_capsule <= 2000]
    if not all(rules):
        continue
    price_for_the_coffee = price_per_capsule * days * needed_capsule
    overall += price_for_the_coffee
